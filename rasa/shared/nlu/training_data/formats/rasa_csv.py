...
class RasaCSVReader(TrainingDataReader):
    ...
    def reads(self, string: Text, **kwargs: Any) -> "TrainingData":
        ...
        """
        csv_content = rasa.shared.utils.io.read_csv_file(string)
        print(csv_content)
        exit()

        if not validation.validate_training_data_format_version(
            csv_content, self.filename
        ):
            return TrainingData()
    ...
    @staticmethod
    def is_csv_nlu_file(filename: Text) -> bool:
        ...
        """
        if not rasa.shared.data.is_likely_csv_file(filename):
            return False
        else:
            return True
