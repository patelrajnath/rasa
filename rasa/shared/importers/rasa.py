...
class MyFileImporter(TrainingDataImporter):
    """Default `TrainingFileImporter` implementation."""
    ...
    def __init__(
        ...
        self.config = autoconfig.get_configuration(config_file, training_type)
        sts_path = self.config.get('importers')[0].get('data_nlu')
        # self._sts_files = None
        self._sts_file_from_nlu = [os.path.join(sts_path, 'train.csv'), os.path.join(sts_path, 'test.csv')]
    ...
    async def get_nlu_data(self, language: Optional[Text] = "en") -> TrainingData:
        utils.training_data_from_paths(self._sts_file_from_nlu, language)
        return utils.training_data_from_paths(self._nlu_files, language)
    ...
    async def get_sts_data(self, language: Optional[Text] = "en") -> TrainingData:
        return utils.training_data_from_paths(self._sts_file_from_nlu, language)
